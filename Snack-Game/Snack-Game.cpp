...
void Setup()
{
	...
	HeadX = Width / 2;
	HeadY = Height / 2;
	//sure that every one program will run ,it generate different numbers
	srand(time(NULL));
...
void input()
{	// check if user press on keyboard or not 
	if (_kbhit())
	{	// save char which user pressed on it
		char C = _getch();
		switch (C)
		{
		case 'w':Dir = UP; break;
...
void logic()
{
	// to put postion of head snake with tails
	// so i saved one postion i will put for head
	int preX, preY;
	int pre2X, pre2Y;
	preX = Tail_X[0];
	preY = Tail_Y[0];
